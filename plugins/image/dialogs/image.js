...
(function () {
    var v = function (d, l) {
        ...
        return {
            ...
            onLoad: function () {
                // 此处为配合下部监听样式修改的标签项，请勿单独删除
                // var tabBox = document.querySelector('div[style~=tabOne]');
                // console.info(document.querySelectorAll('a[role~=tab]'));
                // document.querySelector('a[role~=tab]').parentNode.addEventListener('click',function(){
                //     for(let i = 0 ; i < document.querySelectorAll('a[role~=tab]').length ; i++){
                //         console.info(event.target.getAttribute('id'), document.querySelectorAll('a[role~=tab]')[i].getAttribute('id'));
                //         if(event.target.getAttribute('id') === document.querySelectorAll('a[role~=tab]')[i].getAttribute('id')){
                //             document.querySelectorAll('a[role~=tab]')[i].className = 'cke_dialog_tab cke_dialog_tab_selected';
                //             tabBox.parentNode.querySelectorAll('div[role~='+ tabBox.getAttribute('role') +']')[i].style['display'] = 'block';
                //             tabBox.parentNode.querySelectorAll('div[role~='+ tabBox.getAttribute('role') +']')[i].style['width'] = '100%';
                //         }else{
                //             tabBox.parentNode.querySelectorAll('div[role~='+ tabBox.getAttribute('role') +']')[i].style['display'] = 'none';
                //             document.querySelectorAll('a[role~=tab]')[i].className = 'cke_dialog_tab';
                //         }
                //     }
                // })
                ...
                // 请注意，以下代码为填充图片管理功能。
                // 当图片管理器选中某图片时，触发保存至localstorage事件，此处监听并赋予CKEditor内。
                localStorage.removeItem('customPictures');
                window.addEventListener("storage", function (e) {
                    if(e.key == 'customPictures' && e.newValue.length > 0){
                        // 此处为选项卡索引归位处理
                        document.querySelectorAll('a[role~=tab]').forEach(function(items){
                            items.className = 'cke_dialog_tab';
                        })
                        document.querySelectorAll('a[role~=tab]')[0].className = 'cke_dialog_tab cke_dialog_tab_selected';
                        // 此处为选项卡归位处理
                        tabBox.parentNode.querySelectorAll('div[role~='+ tabBox.getAttribute('role') +']').forEach(function(item){
                            item.style['display'] = 'none';
                        })
                        tabBox.style['display'] = 'block';
                        tabBox.style['width'] = '100%';
                        // 图片属性 预览窗口 选中路径
                        document.querySelector('div[style~=data]').querySelector('input').value = e.newValue;
                        // 图片属性 预览窗口
                        document.querySelector('label#previewIdLabel').parentElement.querySelector('a').href = e.newValue;
                        // 图片属性 预览窗口
                        document.querySelector('label#previewIdLabel').parentElement.querySelector('img').src = e.newValue;
                        // 图片属性 预览窗口 样式更改
                        document.querySelector('label#previewIdLabel').parentElement.querySelector('img').style['display'] = 'inline-block';
                        var imagePic = new Image();
                        imagePic.src= e.newValue;
                        imagePic.onload=function(){
                            // 图片属性 宽度 数值调整
                            document.querySelector('div[style~=dataWidth]').querySelector('input').value = imagePic.width;
                            // 图片属性 高度 数值调整
                            document.querySelector('div[style~=dataHeight]').querySelector('input').value = imagePic.height;
                        };
                        // 清除localstorage现保存数据
                        localStorage.removeItem('customPictures');
                    }
            ...
            onHide: function () {
                if(window.localStorage.uploadPic){
                    window.localStorage.removeItem('uploadPic')
                }
                ...
                elements: [{
                    ...
                    children: [{
                        ...
                        children: [{
                            ...
                            id: "htmlPreview",
                            style: "width:95%;",
                            // 此处 <label id='previewIdLabel'></label> 为配合上部监听的标签项，请勿单独处理
            ...
            // },
            {
                ...
                // 关闭无关功能
                {
                    ...
                    size: 38,
                    onChange: function () {
                        event.target.setAttribute('bbq','ss');
                        var fileDom = event.target;
                        var file = event.target.files[0];
                        window.addEventListener("storage", function (e) {
                            fileDom.value = '';
                        });
                        if(['.png','.gif','.jpg','.jpeg'].includes(file.name.substring(file.name.lastIndexOf('.'),))){
                            // 文件大小判断的处理
                            if( file.size / 1024 / 1024 > 1 ){
                                alert(d.lang.image.picSize);
                                fileDom.value = '';
                                return false;
                            }
                            // 文件分辨率判断的处理
                            ...
                            reader.onload = function (e) {
                                ...
                                image.onload=function(){
                                    ...
                                    if(image.width > 1080 && image.height > 720){
                                        alert(d.lang.image.picMSize);
                                        fileDom.value = '';
                                        return false;
                                    }else{
                                        // 将文件转换为 base64 编码，并保存至 localstorage 里
                                        if (file) {
                                            var name = file.name;
                                            var reader = new FileReader();
                                            reader.readAsDataURL(file);
                                            reader.onload = function(){
                                                ...
                                                localStorage.setItem('uploadPic', JSON.stringify({name: name, value: result}));
                                            }
                                        } else {
                                            localStorage.removeItem('uploadPic');
                                        }
                                    }
                                };
                            };
                            ...
                            event.target.value = '';
                            alert('请上传图片文件（png、gif、jpg、jpeg）');
                        }
                    },
                },
                {
                ...
                elements: [{
                    ...
                    },
                    {
                        ...
                        "default": "vertical-align:top;border:0;",
                        setup: function (a, b) {
                            if (1 == a) {
